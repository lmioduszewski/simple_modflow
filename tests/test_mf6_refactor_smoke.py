...
def test_examples_layout_smoke():
    ...
    # artifacts/ is a gitignored OUTPUT dir — generated by example runs, never
    # tracked — so a fresh clone legitimately lacks it: create, don't assert.
    (examples_dir / "artifacts").mkdir(exist_ok=True)
    ...
    if not sample_dir.exists():
        pytest.skip(
            "examples/mf6/sample_model_output is local-only sample data "
            "(untracked; exists on the author's machine only)"
        )
