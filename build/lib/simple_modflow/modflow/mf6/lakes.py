...
class LakeConnectionData:
    ...
    def __init__(
        ...
        """
        Initializes an instance of the class, setting up the required and optional parameters
        to define a lake configuration and its attributes, connections, and related options.

        :param vor: Object that represents the Voronoi diagram structure.
        :param paths: List of file paths or a single Path object containing lake definitions.
        :param bed_leakance: Value or list representing the leakance of the lake bed.
        :param only_layer: Specific layer to be used; defaults to None if not specified.
        :param horizontal_connections: Dictionary defining horizontal connections for lakes.
        :param use_reconciled_surfaces: Boolean flag indicating whether to use reconciled 
            surfaces for calculations.
        :param alt_surface_df: Dataframe for alternative surfaces. Optional parameter.
        :param min_sep: Minimum separation distance as a floating-point number.
        :param only_vertical: Boolean indicating if only vertical connections should be 
            considered.
        :param verbose: Boolean flag for enabling detailed logging or output.
        :param lake_bathymetry: List of file paths (Path objects) containing lake bathymetric 
            data, if any.
        """
        self.vor = vor
        self._lakes = None
    ...
    def get_connection_data(self):
        ...
        if self.lake_bathymetry is not None:
            return self._get_connection_data_with_bathy(elev_df)

        # Mode 2: rectangular facility with constant top/btm per lake
        if self.horizontal_connections is not None:
            return self._get_connection_data_rectangular(elev_df)
