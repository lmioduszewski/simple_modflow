...
class Budget:
    """
    Class for basic GWF (Groundwater Flow) budget operations.

    This class provides tools to retrieve, process, and analyze budget data for a
    GWF model. Advanced packages such as SFR and LAK have dedicated classes, but
    their data can also be accessed through this class. This class includes methods
    to extract raw budget data, create dataframes from it, and visualize
    area-specific budget observations with advanced plotting capabilities.

    :ivar model: Reference to the simulation model object.
    :type model: SimulationBase
    :ivar gwf_package: Package name for budget data retrieval (e.g., DRN, RCH, GHB).
    :type gwf_package: str
    """
    ...
    def df(self):
        ...
        if self._df is None:
            ...
            for per, bud in enumerate(buds):
                ...
                df = pd.DataFrame(bud)
                # adds a column for a stress period identifier for the data in this budget record
                df['kstpkper'] = [kstpkper for _ in range(len(df))]
                dfs.append(df)

            df = pd.concat(dfs)
            # sets a multiindex for the df to be the first column of the df and the stress period column
            # so you can lookup data by stress period using DataFrame.loc[]
            df = df.set_index([df.columns[0], 'kstpkper'])

            # subtracts 1 from the cell index number, so it is zero-based, not one-based
            # so the index starts at zero to match the indexing of Python and Flopy
            if self.gwf_package in ['drn', 'ghb', 'rch']:
                new_idx = pd.MultiIndex.from_tuples([(i - 1, t) for i, t in df.index])
                df.index = new_idx
                df['node2'] -= 1
    ...
    def plot_budget_obs(
        ...
        if times is not None:
            if not isinstance(times, pd.DatetimeIndex):
                try:
                    ...
                    print('times is not a pandas DatetimeIndex, and cannot convert to datetime')
        else:
          times = None
        ...
        for obs in b_obs.iterrows():
            ...
            for kstpkper in model.kstpkper:
                try:
                    ...
                    bdict[obs[1].loc[name_field]][kstpkper] = flows.loc[cs].sum() / multiplier
                except:
        ...
        if plot_fig or return_fig is True :
            fig = Fig()
            for col in df.columns:
                x = df.index if times is None else times
                fig.add_scattergl(
                    x=x,
                    y=df[col],
                    name=col,
    ...
    def multimodel_plot_budget_obs(
        ...
        :param times: a pandas DatetimeIndex containing the times for each stress periods. If
            provided, it will be plotted on the x-axis, instread of stress period numbers.
        :type times: pd.DatetimeIndex, optional
