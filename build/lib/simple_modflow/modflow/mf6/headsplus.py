...
"""def multimodel_plot_heads(models: list[SimulationBase], locs: int | list[int] | Path, **kwargs):
    ...
    for model in models:
        ...
        for trc in fig.data:
            trc.update(name=f'{trc["name"]}-{model.name}')
        figs.append(fig)
...
def multimodel_plot_heads(models: list["SimulationBase"], locs: int | list[int] | Path, **kwargs):
    """
    Combine head plots for multiple models into one figure, ordering traces so
    traces with the same base name appear next to each other in the legend.
    """
    figures = []
    for model in models:
        fig = model.hds.plot_heads(locs=locs, plot_fig=False, return_fig=True, **kwargs)
    ...
    # Preserve model order within each base trace name
    model_order = {m.name: i for i, m in enumerate(models)}

    grouped: "OrderedDict[str, list[tuple[int, go.BaseTraceType]]]" = OrderedDict()

    for model, fig in zip(models, figures):
        for trc in fig.data:
            base_name = trc.name or "trace"

            # rename + group
            trc = trc.to_plotly_json()  # detach from original figure
            trc["name"] = f"{base_name}-{model.name}"
            trc["legendgroup"] = base_name

            grouped.setdefault(base_name, []).append((model_order[model.name], trc))

    # Build a new figure so we can control trace order freely
    out = figs.Fig()
    out.update_layout(figures[0].layout)

    for base_name, items in grouped.items():
        for _, trc_json in sorted(items, key=lambda x: x[0]):
            out.add_trace(figs.Fig(data=[trc_json]).data[0])
...
class HeadsPlus(bf.HeadFile):
    ...
    def get_obs_cells(self, locs: Path, crs: str = None, loc_name_field='ExploName'):
        ...
        for obs, cell_ids in obs_dict.items():
            ...
            obs_dict[obs] = cell_ids[0][0]  # added a second zero indexer because cell_ids is a list of lists
        return obs_dict
    ...
    def plot_heads(
        ...
        :param show_dates: Boolean flag to indicate whether to use actual model period
            ...
            Defaults to False.
        :param times: Optional pandas DatetimeIndex object containing the simulation times.
        ...
        if locs is not None:
            ...
            if isinstance(locs, Path):
                ...
                assert len(locs) == len(loc_names), 'loc_names must be the same length as locs'
            ...
            for i, obs_loc in enumerate(obs_locs):
                ...
                fig.add_scattergl(
                    x=xs,
                    y=obs_heads,
                    name=loc_names[i] if loc_names is not None else obs_loc,
                )
