...
from simple_modflow.modflow.mf6.grid.triangle import TriangleGrid as Triangle
...
class SimpleModel(SimulationBase):
    ...
    def __init__(
        ...
        self.tdis = TemporalDiscretization(model=self, per_len=30)
        if disv_disu == 'disu':
            self.disu = DisuGrid(vor=self.vor, model=self, top=self.top, bottom=self.bottom)
        if disv_disu == 'disv':
            self.disv = DisvGrid(vor=self.vor, model=self, nlay=nlay, top=self.top, bottom=self.bottom)
        ...
        self.ic = InitialConditions(
            model=self,
            vor=self.vor,
            initial_sat_thickness=self.initial_sat_thickness,
            nlay=self.nlay,
            strt=self.iheads,
        )
        self.k = KFlow(model=self, k=self.k, k33_vert=k33_vert)
        self.oc = OutputControl(model=self)
        self.drn = Drains(model=self, stress_period_data=self.drain_stress_period_data)
        self.sto = Storage(
            model=self,
            specific_yield=0.2,
        ...
        if rch_dict:
            self.rch = Recharge(model=self, vor=self.vor, rch_dict=self.rch_dict)
...
if __name__ == "__main__":
    ...
    examples_dir = Path(__file__).resolve().parents[4] / 'examples' / 'mf6' / 'sample_model_output'
    examples_dir.mkdir(parents=True, exist_ok=True)
    tri.model_ws = examples_dir
    tri.build()

    # get bottom elevs
    pixel_size = 1000  # Pixel size
    l1_botm = examples_dir / 'l1_botm.tif'
    l2_botm = examples_dir / 'l2_botm.tif'
    l3_botm = examples_dir / 'l3_botm.tif'
    top_raster_path = examples_dir / 'top_raster.tif'
    vor = Vor(tri)
