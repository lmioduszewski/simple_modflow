...
class SimulationBase:
    ...
    def __init__(
        ...
        self.ims = flopy.mf6.modflow.mfims.ModflowIms(
            ...
            # Nonlinear controls
            under_relaxation="DBD",
            under_relaxation_theta=0.75,  # 0.7–0.8 typical
            under_relaxation_kappa=0.10,
    ...
    def times(self):
        ...
        if self._times is None:
            times = self.gwf.modeltime.totim
            steps = self.gwf.modeltime.kper_kstp
            per_to_time = {(step[1], step[0]): float(time) for step, time in zip(steps, times)}
            self._times = per_to_time
        return self._times
    ...
    def run_simulation(self):
        ...
        try:
            ...
            print("\nError saving model object to .model file\n")
