"""Build the authoritative package-rich canonical simple_modflow model."""
...
def build_transient_model(
    ...
    *,
    ...
    lake_connections = LakeConnectionData(
        ...
        paths=inputs["lakes"],
        bed_leakance=0.003,
        horizontal_connections={0: [123.0, 108.0], 1: [121.0, 106.0]},
    ...
    reach_cells = sfr.stream_cells[0]
    ...
    sfr.reach_elevations = [reach_elevations.tolist()]
    sfr.widths = [[24.0 + 10.0 * value for value in reach_fraction]]
    sfr.rhk = [[0.004 + 0.014 * abs(np.sin(3.0 * np.pi * value)) for value in reach_fraction]]
    sfr.add_sfr()
    outlet_reach = int(sfr.stream_reaches[0][-1] - 1)
    mvr_perioddata = {
        period: [
            ["sfr", outlet_reach, "lak", 0, "FACTOR", 0.20],
            ["sfr", outlet_reach, "lak", 1, "FACTOR", 0.15],
        ]
        for period in range(config.nper)
    }
    MVR(
...
def _attach_canonical_targets(
    ...
    *,
    ...
    reaches = [int(value - 1) for value in sfr.stream_reaches[0]]
