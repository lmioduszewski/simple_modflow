...
def _maybe_create_package_artifact(
    ...
    *,
    ...
    This is a convenience path for catalog-managed workflows: the package can be
    defined normally, and if an `artifact_id` is provided the package is also
    captured into the catalog and recorded on the model's run manifest.
    """

    if artifact_id is None:
        return None

    catalog = artifact_catalog or getattr(model, "project_catalog", None)
    if catalog is None:
        raise ValueError("artifact_catalog or model.project_catalog is required when artifact_id is provided.")
    ...
    model.register_package_reference(package_name.lower(), artifact.artifact_id)
    return artifact
...
class LAK:
    """Create the MF6 LAK package, optionally as a reusable artifact."""
    ...
    def __init__(
        ...
        ----------
        model
            Target model receiving the LAK package.
        nlakes, noutlets, ntables
        ...
        validate
            Whether to run LAK input validation against the current model
            before attaching the package.
        artifact_id, artifact_catalog, artifact_description, artifact_tags,
        artifact_metadata, artifact_overwrite
            Optional reusable-artifact capture settings.
        """
        self.validation_report = validate_lak_configuration(
            model,
            nlakes=nlakes,
...
class MVR:
    ...
    def __init__(
        ...
        """Create an MF6 MVR package and optionally capture it as a reusable artifact.

        Parameters
        ----------
        model
            Target model receiving the MVR package.
        maxmvr, maxpackages
            Core MF6 MVR sizing parameters.
        packages, perioddata
            FloPy/MF6 mover package definitions and period records.
        print_input, print_flows, modelnames
            Standard MF6 MVR control flags.
        validate
            Whether to validate MVR package references and zero-based mover ids
            before attaching the package.
        artifact_id, artifact_catalog, artifact_description, artifact_tags,
        artifact_metadata, artifact_overwrite
            Optional reusable-artifact capture settings.
        """
        self.validation_report = validate_mvr_configuration(
            model,
            maxmvr=maxmvr,
        ...
        if validate:
            self.validation_report.raise_for_errors("MVR validation failed.")

        self.mvr = flopy.mf6.ModflowGwfmvr(
            model.gwf,
            print_input=print_input,
            print_flows=print_flows,
