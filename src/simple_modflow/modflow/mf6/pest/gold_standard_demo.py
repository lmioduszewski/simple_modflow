...
preferred ``simple_modflow`` workflow from end to end:
...
from simple_modflow.modflow.mf6.pest.project import PestProject
from simple_modflow.modflow.mf6.pest.results import open_pest_run
...
def _build_empty_targets(
    *,
    ...
    main_reaches = [reach - 1 for reach in sfr.stream_reaches[0]]
    sfr_stage_locations = {
...
def _build_model(
    *,
    ...
    lake_connections = LakeConnectionData(
        ...
        paths=lake_paths,
        bed_leakance=0.12,
        horizontal_connections={0: [config.north_lake_stage_start + 0.3, config.north_lake_stage_start - 7.5],
                                1: [config.south_lake_stage_start + 0.2, config.south_lake_stage_start - 7.0]},
    ...
    lake_packagedata = LakePackageData(
        nlakes=2,
        starting_stage=[config.north_lake_stage_start, config.south_lake_stage_start],
        connectiondata=connectiondata,
        boundnames=["north_lake", "south_lake"],
    )
    lake_perioddata = LakePeriodData(
        model=model,
        lake_ids=[0, 1],
        status={0: ["ACTIVE"] * config.nper, 1: ["ACTIVE"] * config.nper},
    )
    LAK(
        model=model,
        nlakes=2,
    ...
    mvr_perioddata = {
        int(per): [
            ["sfr", int(sfr.stream_reaches[0][-1] - 1), "lak", 0, "FACTOR", 0.60],
            ["sfr", int(sfr.stream_reaches[0][-1] - 1), "lak", 1, "FACTOR", 0.40],
        ]
        for per in range(config.nper)
    }
    MVR(
    ...
    lake_cells = {
        int(cell)
        for values in lake_connections.lakes_vor_cells.values()
        for cell in values
    }
    stream_cells = {int(cell) for cells in sfr.stream_cells for cell in cells}
