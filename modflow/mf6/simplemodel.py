...
if __name__ == "__main__":
    ...
    tri.add_circle(radius=5000, center_coords=(0, 0))
    ...
    for uic in uics:
        c = vor.get_vor_cells_as_series(uic).to_list()
        ssb_cells = c

        facility_area = vor.get_overlapping_area(cell_list=ssb_cells)
        rch_trans = [(19_250 * 1) / facility_area for i in range(nper)]  # 100 gpm for one UIC
